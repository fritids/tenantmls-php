...
class SQLQuery {
			...
			while ($row = mysql_fetch_row($this->_result)) {
				...
				if ($this->_hMABTM == 1 && isset($this->hasManyAndBelongsToMany)) {
					foreach ($this->hasManyAndBelongsToMany as $aliasChild => $tableChild) {
						...
						$fromChild .= '`'.$tableChild.'` as `'.$aliasChild.'`,';
						$fromChild .= '`'.$joinTable.'`,';
						// Modified for locales since the table is indexed by ZIP not ID
						//if ($tableChild == 'locales')
							//$conditionsChild .= '`'.$joinTable.'`.`'.$singularAliasChild.'_zip` = `'.$aliasChild.'`.`city_zip` AND ';
						//else
	...
	function custom($query,$last_insert_id = false) {
	...
	function save($last_insert_id = false) {
		$query = '';
		if (isset($this->id)) {
			$updates = '';
			foreach ($this->_describe as $field) {
